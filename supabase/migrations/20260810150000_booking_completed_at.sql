-- Work session v2:
--   1. bookings.completed_at: actual finish timestamp, set by set_booking_status
--      when a confirmed booking transitions to 'completed' (actual duration =
--      completed_at - started_at). Nullable; other transitions clear it.
--   2. set_booking_status: additive change only — same auth checks, same transition
--      matrix; only writes completed_at alongside the status.
...
comment on column public.bookings.completed_at is
  'Actual finish time, set when the barber completes the booking (null otherwise).';

create index bookings_staff_completed_idx
  on public.bookings (staff_id, completed_at)
  where completed_at is not null;

-- 2. set_booking_status with finish timestamp ------------------------------
