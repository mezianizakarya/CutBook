-- A barber can only join a shop once they have entered professional data
-- (specialty / years of experience). These columns are written synchronously by
-- the barber-professional onboarding screen, so they are DB-authoritative and
-- immediately available. onboarding_step is webhook-mirrored from Clerk
-- (eventual) and is only a progress marker, so it must NOT gate this action.
...
begin
  ...
  if v_profile.specialty is null and v_profile.years_of_experience is null then
