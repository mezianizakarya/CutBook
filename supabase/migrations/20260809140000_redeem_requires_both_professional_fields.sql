-- ============================================================================
-- Join-shop gate requires BOTH required professional fields.
--
-- Previously the RPC accepted a barber with specialty OR years_of_experience.
-- A barber professional profile is complete only when BOTH are present:
--   * specialty is set and non-blank
--   * years_of_experience is set (0..100 enforced by the existing CHECK)
-- Bio stays optional and never gates joining a shop.
--
-- The invitation row is still locked inside the RPC, so a single-use code can
-- never be redeemed twice even under concurrency.
-- ============================================================================
...
begin
  ...
  if v_profile.specialty is null
     or btrim(v_profile.specialty) = ''
     or v_profile.years_of_experience is null then
...
-- Defense in depth: an empty/blank specialty can never be stored as if it were
-- complete. The RPC gate above rejects blanks too. (Live data has none.)
alter table public.profiles
  add constraint profiles_specialty_not_blank
